...
function BookAppointment() {
    ...
    useEffect(() => {
        const url = `http://65.1.109.236:5000/allDoctorData/${email}`
    ...
    const getDoctorSlotData = (doctorEmail) => {
        const url = `http://65.1.109.236:5000/getDoctorSlot/${doctorEmail}`
        axios.get(url).then(response => {
            console.log(response)
    ...
    const submitSelectedRow = () => {
        ...
        const url = `http://65.1.109.236:5000/updateDoctorSlotData/${selectedRows[0]._id}`
